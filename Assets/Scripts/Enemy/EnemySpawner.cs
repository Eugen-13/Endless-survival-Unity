...
public class EnemySpawner : MonoBehaviour
{
    ...
    [SerializeField] private List<EnemySpawnWeight> _spawnWeights;
    ...
    private void Start()
    {
        ...
        foreach (EnemySpawnWeight item in _spawnWeights)
        {
    ...
    {
        float sum = _spawnWeights.Sum(e => e.Weight);
        float rand = Random.Range(0, sum);
        foreach (EnemySpawnWeight weight in _spawnWeights)
        {
            rand -= weight.Weight;
            if (rand <= 0)
            {
